...
protocol SocketServiceProtocol {
    ...
    func sendMessage(receiverId: String, content: String, role: SenderRole?)
}
...
final class SocketService: SocketServiceProtocol {
    ...
    func sendMessage(receiverId: String, content: String, role: SenderRole? = nil) {
        guard isAuthenticated else {
            print("⚠️ Cannot send message: Not authenticated")
            return
        }
    ...
    private func setupEventHandlers() {
        ...
        // Message events
        socket.on(SocketEvents.message) { [weak self] data, ack in
            self?.handleMessage(data: data)
        }

        socket.on("messageSent") { [weak self] data, ack in
    ...
    private func handleAuthenticationSuccess(data: [Any]) {
        ...
        if let jsonData = data.first as? [String: Any],
            ...
            let authSuccess = try? JSONDecoder().decode(
                AuthenticationSuccessData.self, from: userData)
        {
    ...
    private func handleTokenRefreshSuccess(data: [Any]) {
        ...
        if let jsonData = data.first as? [String: Any],
            let userData = try? JSONSerialization.data(withJSONObject: jsonData),
            let refreshData = try? JSONDecoder().decode(
                TokenRefreshSuccessData.self, from: userData)
        {
    ...
    private func handleTokenExpirationWarning(data: [Any]) {
        if let jsonData = data.first as? [String: Any],
            let userData = try? JSONSerialization.data(withJSONObject: jsonData),
            let warningData = try? JSONDecoder().decode(
                TokenExpirationWarningData.self, from: userData)
        {
    ...
    private func handleMessage(data: [Any]) {
        ...
        {
            print("📩 Message received from \(message.senderId)")
    ...
    private func handleMessageSent(data: [Any]) {
        if let jsonData = data.first as? [String: Any],
            let messageData = try? JSONSerialization.data(withJSONObject: jsonData),
            let message = try? JSONDecoder().decode(Message.self, from: messageData)
        {
...
extension Notification.Name {
    ...
    static let socketMessageSent = Notification.Name("socketMessageSent")
}
